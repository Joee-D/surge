...
function getcontent() {
    ...
    for (var i = 1; i <= 365; i++) {
        ...
        if ((tmp.festival != null) && (count[0] < showcount)) {
            content[0] += tmp.festival.padEnd(9 - tmp.festival.length)+':'+i+'天\t ';
            count[0]++;
        } else if ((tmp.lunarFestival != null) && (count[1] < showcount)) {
            content[1] += tmp.lunarFestival.padEnd(9 - tmp.lunarFestival.length)+':'+i+'天\t ';
            count[1]++;
        } else if ((tmp.Term != null) && (count[2] < showcount)) {
            content[2] += tmp.Term.padEnd(9 - tmp.Term.length)+':'+i+'天\t ';
            count[2]++;
        }
